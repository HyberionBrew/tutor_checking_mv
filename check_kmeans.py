...
if __name__ == '__main__':
    ...
    start = time.time()
    centers, labels = student_module.kmeans(points,
    ...
    # Check if kmeans can handle empty clusters:
    try:
        centers, labels =  student_module.kmeans(points,
                                    n_clusters=2,
                                    n_iterations=1,
                                    centers_in=np.array([[0,0,0],[1000,1000,100]]),
                                    max_singlerun_iterations=max_singlerun_iterations)
        print(f"K-means, empty cluster check. This should return 1 cluster. Number of clusters: {len(set(labels))}")
    except:
        print("Student program crashes with k-means and bad centers_in."
                "The reason might be that it cannot handle empty clusters.")
    """
